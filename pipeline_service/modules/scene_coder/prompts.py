...
# --- Router support -----------------------------------------------------------
# The system prompt above carries three object-class handbooks. The router picks
# which ones a given reference image needs; the rest are cut out so the coder's
# context is focused on the object at hand. With every handbook selected (or with
# `categories=None`) the result is byte-identical to CODER_SYSTEM_PROMPT.
_HANDBOOK_MARKERS: dict[str, str] = {
    "seating": "Seating furniture / upholstery handbook:",
    "decal": "Surface decoration / decal handbook:",
    "vehicle": "Vehicle modeling playbook:",
}
_HANDBOOK_ORDER: tuple[str, ...] = ("seating", "decal", "vehicle")
_HANDBOOK_END_MARKER = "Proportion tuning shortcut:"


def build_coder_system_prompt(categories: set[str] | None) -> str:
    if categories is None:
        return CODER_SYSTEM_PROMPT
    text = CODER_SYSTEM_PROMPT
    spans: list[tuple[str, int, int]] = []
    for i, key in enumerate(_HANDBOOK_ORDER):
        start = text.find(_HANDBOOK_MARKERS[key])
        nxt = _HANDBOOK_MARKERS[_HANDBOOK_ORDER[i + 1]] if i + 1 < len(_HANDBOOK_ORDER) else _HANDBOOK_END_MARKER
        end = text.find(nxt)
        if start < 0 or end < 0 or end <= start:
            return CODER_SYSTEM_PROMPT  # prompt layout changed -> never cut blindly
